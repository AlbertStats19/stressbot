...
def generate_transactions_data(num_records, company_names_original):
...
def generate_financial_data(num_records, company_names_original):
    """Genera datos simulados de métricas financieras."""
    print(f"Generando {num_records} registros financieros...")
    data = {
...
def main():
    ...
    # Generar data de transacciones
    with timer("Generación de data de transacciones"):
        transactions_df = generate_transactions_data(num_records=10_000_000, company_names_original=company_names_original)
        transactions_df.to_csv(os.path.join(output_data_dir, 'simulated_transactions.csv'), index=False)
    print("Data de transacciones guardada.")

    # Generar data financiera
    with timer("Generación de data financiera"):
        financial_df = generate_financial_data(num_records=300_000, company_names_original=company_names_original)
        financial_df.to_csv(os.path.join(output_data_dir, 'simulated_financial_metrics.csv'), index=False)
    print("Data financiera guardada.")
